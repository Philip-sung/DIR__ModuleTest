...
if (args[0] === "allTeam") {
    const uniqueTeamName = Array.from(new Set(ImagoMembersWithInfo.map((member) => member.position)));
...
if (!Number.isNaN(Number(args[0]))) {
    ...
    if (args[1] === undefined) {
        ...
        candidate = ImagoMembersWithInfo.filter((member) => member.position === args[1]);
    }
    else {
        candidate = ImagoMembersWithInfo.filter((member) => member.position.includes(args[1]));
    }
